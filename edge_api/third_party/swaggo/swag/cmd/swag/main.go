...
const docTemplate = ` + "`" + `{
  ...
  "basePath": "/",
  "paths": {
    "/healthz": {"get": {"summary": "Liveness probe", "produces": ["application/json"], "responses": {"200": {"description": "OK"}}}},
    "/readyz": {"get": {"summary": "Readiness probe", "produces": ["application/json"], "responses": {"200": {"description": "Ready"}, "503": {"description": "Not ready"}}}},
...
func main() {
	if len(os.Args) < 2 || os.Args[1] != "init" {
		fmt.Fprintln(os.Stderr, "usage: swag init -o <dir>")
		os.Exit(2)
	}
	fs := flag.NewFlagSet("init", flag.ExitOnError)
	output := fs.String("o", "docs", "output directory")
	_ = fs.String("g", "", "main file")
	_ = fs.Bool("parseInternal", false, "compat")
	_ = fs.Parse(os.Args[2:])
	if err := os.MkdirAll(*output, 0o755); err != nil {
		fmt.Fprintln(os.Stderr, err)
		os.Exit(1)
	}
	if err := os.WriteFile(filepath.Join(*output, "docs.go"), []byte(docsGo), 0o644); err != nil {
		fmt.Fprintln(os.Stderr, err)
		os.Exit(1)
	}
}
