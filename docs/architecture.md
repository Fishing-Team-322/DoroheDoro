...
- потоковым автоматическим детектором редких fingerprint-ов (на базе ClickHouse + normalized events)
...
Внутри query-alert-plane живёт системный детектор логов с редкими fingerprint-ами: он подписывается на `logs.ingest.normalized`, отслеживает частоту `(host, service, severity, fingerprint)` через ClickHouse за скользящее окно и, если счётчик не превышает заданный порог, поднимает `alert_instance` с системным правилом `[system] rare fingerprint detector`. Все параметры (включая длительность окна и максимальное число повторов) управляются env-переменными `RARE_FINGERPRINT_*`.
